...
<property>
  <name>dfs.name.dir</name>
    <value>file:///data/hadoop-3.4.1/namenode</value>
</property>

<property>
  <name>dfs.data.dir</name>
    <value>file:///data/hadoop-3.4.1/datanode</value>
</property>
...
<configuration>
 ...
 <property>
    ...
    <value>HADOOP_MAPRED_HOME=/data/hadoop-3.4.1</value>
 </property>
 <property>
...
if __name__ == '__main__':
    ...
    os.system("rm -rf /data/hadoop-3.4.1.tar.gz")
    ...
    os.system("cd /data && wget https://mirrors.tuna.tsinghua.edu.cn/"
              "apache/hadoop/common/hadoop-3.4.1/hadoop-3.4.1.tar.gz --no-check-certificate")
    print("hadoop3.4.1 下载完成")
    # 解压安装包
    os.system("cd /data && tar -zxvf jdk8.tar.gz")
    os.system("cd /data && tar -zxvf hadoop-3.4.1.tar.gz")
    # os.system("mkdir -p ")
    ...
    # 修改主机名
    os.system("hostname hadoop341")
    os.system("echo 'hadoop341'>/etc/hostname")
    # ssh免密
    ...
    # 3. 修改hadoop相关配置文件
    with open("/data/hadoop-3.4.1/etc/hadoop/hadoop-env.sh", "w") as f:
        f.write(hadoop_env_sh_content)
    with open("/data/hadoop-3.4.1/etc/hadoop/core-site.xml", "w") as f:
        f.write(core_site_content)
    with open("/data/hadoop-3.4.1/etc/hadoop/hdfs-site.xml", "w") as f:
        f.write(hdfs_site_content)
    with open("/data/hadoop-3.4.1/etc/hadoop/yarn-site.xml", "w") as f:
        f.write(yarn_site_content)
    with open("/data/hadoop-3.4.1/etc/hadoop/mapred-site.xml", "w") as f:
        f.write(mapred_site_content)


    # 4. 修复root用户无法执行start-dfs.sh和 stop-dfs.sh  start-yarn.sh  stop-yarn.sh
    # start-dfs.sh修复，允许root用户
    with open("/data/hadoop-3.4.1/sbin/start-dfs.sh", "r") as f:
    ...
    if sbin_dfs_prefix not in start_dfs_content:
        start_dfs_content = start_dfs_content.replace('#!/usr/bin/env bash\n',
                                                      '#!/usr/bin/env bash\n' + sbin_dfs_prefix+'\n')
    with open("/data/hadoop-3.4.1/sbin/start-dfs.sh", "w") as f:
        f.write(start_dfs_content)
    # stop-dfs.sh修复，允许root用户
    with open("/data/hadoop-3.4.1/sbin/stop-dfs.sh", "r") as f:
    ...
    if sbin_dfs_prefix not in stop_dfs_content:
        stop_dfs_content = stop_dfs_content.replace('#!/usr/bin/env bash\n',
                                                      '#!/usr/bin/env bash\n' + sbin_dfs_prefix + '\n')
    with open("/data/hadoop-3.4.1/sbin/stop-dfs.sh", "w") as f:
        f.write(stop_dfs_content)
    # start-yarn.sh修复，允许root用户
    with open("/data/hadoop-3.4.1/sbin/start-yarn.sh", "r") as f:
    ...
    if sbin_yarn_prefix not in start_yarn_content:
        start_yarn_content = start_yarn_content.replace('#!/usr/bin/env bash\n',
                                                      '#!/usr/bin/env bash\n' + sbin_yarn_prefix +'\n')
    with open("/data/hadoop-3.4.1/sbin/start-yarn.sh", "w") as f:
        f.write(start_yarn_content)
    # stop-yarn.sh修复，允许root用户
    with open("/data/hadoop-3.4.1/sbin/stop-yarn.sh", "r") as f:
    ...
    if sbin_yarn_prefix not in stop_yarn_content:
        stop_yarn_content = stop_yarn_content.replace('#!/usr/bin/env bash\n',
                                                      '#!/usr/bin/env bash\n' + sbin_yarn_prefix + '\n')
    with open("/data/hadoop-3.4.1/sbin/stop-yarn.sh", "w") as f:
    ...
    if './start-dfs.sh' not in _:
        os.system("echo 'cd /data/hadoop-3.4.1/sbin && ./start-dfs.sh'>> /etc/rc.local")
    if './start-yarn.sh' not in _:
        os.system("echo 'cd /data/hadoop-3.4.1/sbin && ./start-yarn.sh'>> /etc/rc.local")
    print("1. 请执行如下命令初始化hdfs:\n"
          "source /etc/profile \n"
          "export JAVA_HOME=/data/jdk8u452-b09\n"
          "cd /data/hadoop-3.4.1/bin && ./hdfs namenode -format\n"
          "2. 请执行如下命令启动hdfs及yarn：\n"
          "cd /data/hadoop-3.4.1/sbin && ./start-dfs.sh\n"
          "cd /data/hadoop-3.4.1/sbin && ./start-yarn.sh")
